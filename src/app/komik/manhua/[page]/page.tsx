...
import { fetchManhua, type MangaItem } from "@/lib/api/komik"
    ...
    <div className="flex items-center justify-center gap-4 mt-20 animate-fade-in">
      {has_previous_page ? (
        <Button 
          ...
          variant="secondary"
          className="rounded-xl px-6 py-2 border border-border/10 hover:border-red-500/50 transition-all font-black uppercase text-[10px] tracking-widest"
        >
      ...
      <div className="flex items-center gap-2 px-6 py-2 bg-card/40 backdrop-blur-md rounded-xl border border-border/10 font-bold text-xs uppercase tracking-tighter">
        Page <span className="text-red-500">{current_page}</span> / {last_visible_page || "?"}
      </div>

      {has_next_page ? (
        <Button 
          href={`${baseUrl}/${current_page + 1}`}
          className="rounded-xl px-6 py-2 font-black uppercase text-[10px] tracking-widest bg-red-600 hover:bg-red-700 shadow-lg shadow-red-500/20"
        >
...
function KomikCard({ item, index }: { item: MangaItem; index: number }) {
  return (
    <Link href={`/komik/detail/${item.slug}`} className="group relative block h-full animate-slide-up">
       <Card className="relative p-0 overflow-hidden aspect-[3/4.2] border-white/5 group-hover:border-red-500/50 transition-all duration-500 shadow-2xl bg-card">
        <CachedImage
          src={item.poster}
          alt={item.title}
          fill
        ...
        <div className="absolute top-3 left-3 right-3 flex justify-between items-start">
           <Badge variant="glass" className="bg-red-500/20 text-red-400 border-red-500/30 text-[9px] font-black uppercase">
             {item.type || "Manhua"}
           </Badge>
           {item.score && (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 space-y-2 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-red-500/20 border border-red-500/30 text-[9px] font-black uppercase text-red-400">
             {item.chapter}
          </div>
          <h3 className="text-sm md:text-base font-bold text-white leading-tight line-clamp-2 transition-colors group-hover:text-red-400">
            {item.title}
          </h3>
        </div>
      </Card>
...
function ListContent({ page }: { page: number }) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["manhua-list", page],
    queryFn: () => fetchManhua(page)
  })
  ...
  if (error || !data) return (
    <div className="p-20 text-center glass rounded-[3rem] border border-red-500/20">
        ...
        <p className="text-muted-foreground mt-2">The neural uplink for Manhua library failed.</p>
    </div>
  )
  ...
  return (
    <div className="space-y-20">
      ...
      <PaginationControl pagination={data.pagination} baseUrl="/komik/manhua" />
    </div>
  )
}
    ...
    <main className="min-h-screen bg-background text-foreground transition-colors duration-500 pb-40">
      <div className="relative h-[30rem] w-full flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-20 text-center px-4 space-y-6">
          <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-red-500/10 backdrop-blur-md border border-red-500/20 text-[10px] font-black uppercase tracking-[0.4em] text-red-400">
             <IconBook size={14} className="animate-pulse" /> Chinese Archive
          </div>
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter uppercase italic text-foreground">
            MANHUA<span className="text-red-500">PROTOCOL</span>
          </h1>
          <p className="max-w-xl mx-auto text-muted-foreground/60 font-medium text-sm md:text-base tracking-wide">
             Accessing legendary Chinese literary transmissions and high-fidelity scroll data.
          </p>
        </div>
      </div>
      ...
      <Section className="px-6 py-20">
         <div className="mb-12">
            <Link 
              href="/komik" 
              className="inline-flex items-center gap-2 text-red-500 hover:text-foreground transition-colors text-[10px] font-black uppercase tracking-[0.3em]"
            >

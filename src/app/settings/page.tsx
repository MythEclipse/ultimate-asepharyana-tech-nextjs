    ...
    <main className="min-h-screen relative overflow-hidden pb-40">
      ...
      <div className="container mx-auto max-w-3xl px-8 pt-24 space-y-16 relative z-10">
        ...
        <header className="text-center space-y-6 animate-fade-in">
          <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full glass border border-white/10 text-[10px] font-black uppercase tracking-[0.4em] text-primary">
            System Parameters
          </div>
        ...
        <div className="space-y-12">
          ...
          <section className="glass-card rounded-[3rem] p-10 border border-white/5 animate-slide-up [animation-delay:200ms] group hover:border-white/20 transition-all duration-700">
            <div className="flex items-center gap-4 mb-10">
              <div className="w-12 h-12 rounded-2xl bg-primary/20 flex items-center justify-center text-2xl shadow-2xl">🧠</div>
              <div className="space-y-1">
                <h2 className="text-2xl font-black italic tracking-tighter uppercase">Profile <span className="text-muted-foreground/40">Identity</span></h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2 p-6 rounded-2xl bg-white/2 border border-white/5">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Alias</label>
                <p className="text-lg font-black italic tracking-tight">{user?.name || ""}</p>
              </div>
              <div className="space-y-2 p-6 rounded-2xl bg-white/2 border border-white/5">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Uplink ID</label>
                <p className="text-lg font-black italic tracking-tight">{user?.email || ""}</p>
              </div>
            </div>
          </section>
          
          {/* Chromatic Architecture Section */}
          <section className="glass-card rounded-[3rem] p-10 border border-white/5 animate-slide-up [animation-delay:400ms] group hover:border-white/20 transition-all duration-700">
            <div className="flex items-center gap-4 mb-10">
              <div className="w-12 h-12 rounded-2xl bg-blue-500/20 flex items-center justify-center text-2xl shadow-2xl">🎨</div>
              <div className="space-y-1">
                <h2 className="text-2xl font-black italic tracking-tighter uppercase">Chromatic <span className="text-muted-foreground/40">Engine</span></h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <button
                ...
                className={`relative group/btn p-8 rounded-[2rem] glass transition-all duration-500 active:scale-95 ${
                    theme === "light" ? "bg-blue-500/10 border-blue-500/40 text-blue-400" : "border-white/5 text-muted-foreground hover:bg-white/5 hover:text-foreground"
                }`}
              >
              ...
              <button
                ...
                className={`relative group/btn p-8 rounded-[2rem] glass transition-all duration-500 active:scale-95 ${
                    theme === "dark" ? "bg-indigo-500/10 border-indigo-500/40 text-indigo-400" : "border-white/5 text-muted-foreground hover:bg-white/5 hover:text-foreground"
                }`}
              >
                <div className="relative z-10 space-y-3">
              ...
              <button
                ...
                className={`relative group/btn p-8 rounded-[2rem] glass transition-all duration-500 active:scale-95 ${
                    theme === "system" ? "bg-purple-500/10 border-purple-500/40 text-purple-400" : "border-white/5 text-muted-foreground hover:bg-white/5 hover:text-foreground"
                }`}
              >
                <div className="relative z-10 space-y-3">
                  <span className="text-3xl block transition-transform group-hover/btn:scale-125 duration-500">💻</span>
          ...
          <section className="glass-card rounded-[3rem] p-10 border border-red-500/10 bg-red-500/5 animate-slide-up [animation-delay:600ms] group hover:border-red-500/30 transition-all duration-700">
             <div className="flex items-center gap-4 mb-10">

...
// API & UI
import { fetchAnime1Ongoing, fetchAnime1Complete } from "@/lib/api/anime"
...
function AnimeHubContent({ source }: { source: 1 | 2 }) {
  const { data: ongoing } = useQuery({ queryKey: ["ongoing", source], queryFn: () => fetchAnime1Ongoing(1) })
  const { data: complete } = useQuery({ queryKey: ["complete", source], queryFn: () => fetchAnime1Complete(1) })
      ...
      <div className="relative h-[40rem] w-full flex flex-col items-center justify-center overflow-hidden rounded-md">
        ...
        <div className="relative z-20 text-center px-4">
          ...
          <div className="mt-10 max-w-xl mx-auto relative group">
            ...
            <form action="/anime/search" className="relative flex items-center bg-card/80 backdrop-blur-xl border border-border/50 rounded-2xl p-1 shadow-2xl transition-all">

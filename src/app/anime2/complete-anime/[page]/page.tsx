...
import { fetchAnime2Complete, type Anime2CompleteItem } from "@/lib/api/anime"
...
function CompleteListContent({ page }: { page: number }) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["anime2-complete", page],
    queryFn: () => fetchAnime2Complete(page)
  })
  ...
  if (error || !data) return (
    <div className="p-20 text-center glass rounded-[3rem] border border-purple-500/20">
        ...
        <p className="text-muted-foreground mt-2">The secondary neural uplink for complete library failed.</p>
    </div>
  )
  ...
  return (
    <div className="space-y-20">
      ...
      <PaginationControl pagination={data.pagination} baseUrl="/anime2/complete-anime" />
    </div>
  )
}

...
function OngoingAnimeCard({ item }: { item: Anime2OngoingItem }) {
  return (
    <Link href={`/anime2/detail/${item.slug}`} className="group relative block h-full animate-slide-up">
       <Card className="relative p-0 overflow-hidden aspect-[3/4.2] border-white/5 group-hover:border-primary/50 transition-all duration-500 shadow-2xl">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 space-y-2 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
          <h3 className="text-sm md:text-base font-bold text-white leading-tight line-clamp-3 transition-colors group-hover:text-primary">
            {item.title}
          </h3>
        </div>
      </Card>
...
function OngoingListContent({ page }: { page: number }) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["anime2-ongoing", page],
    queryFn: () => fetchAnime2Ongoing(page)
  })
  ...
  if (error || !data) return (
    <div className="p-20 text-center glass rounded-[3rem] border border-red-500/20">
        ...
        <p className="text-muted-foreground mt-2">The secondary neural uplink for ongoing series list failed.</p>
    </div>
  )
  ...
  return (
    <div className="space-y-20">
      ...
      <PaginationControl pagination={data.pagination} baseUrl="/anime2/ongoing-anime" />
    </div>
  )
}
